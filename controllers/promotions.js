...
exports.getPromotions = async (req, res) => {
  try {
    const result = await db.query(
      `SELECT id, title, body, image_url AS "imageUrl", link_url AS "linkUrl", created_at AS "createdAt"
       FROM promotions
  ...
  try {
    await db.query(
      `INSERT INTO promotions (id, title, body, image_url, link_url)
       VALUES ($1, $2, $3, $4, $5)`,
      [id, title, body, imageUrl || null, linkUrl || null]
    );
...
exports.updatePromotion = async (req, res) => {
  ...
  try {
    ...
    await db.query(
      `UPDATE promotions
       SET title = $1, body = $2, image_url = $3, link_url = $4
       WHERE id = $5`,
      [title, body, imageUrl || null, linkUrl || null, id]
    );
